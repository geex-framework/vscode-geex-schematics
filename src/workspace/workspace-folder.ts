...
import { GeexConfig } from './GeexConfig';
...
export class WorkspaceFolderConfig implements vscode.WorkspaceFolder {
  ...
  async init(): Promise<void> {
    ...
    if (geexPatternMatches.length) {
      this.geexConfigUri = await this.findGeexConfigUri({
        uri: this.uri,
        name: this.name,
        index: this.index,
      });

      const geexConfig = await new GeexConfig().init(this.geexConfigUri);
      this.geexConfig = geexConfig;
    }

    const angularPattern = new vscode.RelativePattern(this, "**/angular.json");
    // todo: find better way to identify the angular folder
    let angularPatternMatches = await vscode.workspace.findFiles(angularPattern, '**/node_modules/**', 1);
    if (angularPatternMatches.length == 0) {
      return;
    }
  ...
  /**
     * Try to find the Angular config file's fs path, or `undefined`
     */
  private async findGeexConfigUri(workspaceFolder: vscode.WorkspaceFolder): Promise<vscode.Uri> {

    /* Search in root folder only first for performance */

    /* Required to look only in the current workspace folder (otherwise it searches in all folders) */
    const rootPattern = new vscode.RelativePattern(workspaceFolder, "**/*.code-workspace");
    ...
    if (searchMatches.length > 0) {
      ...
      if (searchMatches.length === 1) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
        Output.logInfo(`folder for modules folder found at: ${searchMatches[0]!.path}`);

      } else {

        const errorMessage = `More than one folder found for modules folder. If you work with multiple repositories at once, you need to open them as different workspaces in Visual Studio Code.`;
